...
window.onload = () => {
  ...
  currentFilteredProducts = JSONObject.product.filter(
    (p) => p.id !== "0000000"
  );
  ...
  if (categoryFromURL) {
    const checkbox = document.querySelector(
      `.filterForm input[value="${categoryFromURL.toLowerCase()}"]`
    );
    if (checkbox) {
...
function applyFilters() {
  const searchText = document
    .getElementById("searchInputTop")
    .value.toLowerCase();
  const checkboxes = document.querySelectorAll(
    '.filterForm input[type="checkbox"]:checked'
  );
  const selectedCategories = Array.from(checkboxes).map((cb) =>
